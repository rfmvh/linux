...
static int setup_filter(const struct dirent *entry){
	return strcmp(entry->d_name, "setup.sh");
}
...
{
	const struct shell_info *test_info = test->priv;
	const char *file;
	int err;
	char *cmd = NULL;

	/* Get absolute file path */
	if (subtest >= 0) {		/* Test suite */
		file = test->test_cases[subtest].name;
	}		/* Single test case */
	else {
		file = test->test_cases[0].name;
	}
	// if (asprintf(&cmd, "%s%s%s", dirpath, file, verbose ? " -v" : "") < 0)
	// 	return TEST_FAIL;

	if (chdir(test_info->path))
		return TEST_FAIL;

	// if has setup run setup and subtest is equal to 0
	if (subtest == 0 && test_info->has_setup){
		err = system("./setup.sh");
		if (err)
...
static struct test_suite* prepare_test_suite(int dir_fd)
{
	...
	test_info->path = strdup_check(dirpath);		/* Absolute path to dir */
	test_info->has_setup = false;
...
static void append_script_to_suite(const char *name, char *desc,
					struct test_suite **test_suite,
					size_t *tc_count, const char *subpath)
{
	...
	if (asprintf(&file_name, "%s%s", subpath, name) < 0) {
		pr_err("Out of memory while building script test suite list\n");
		return;
	}

	tests[(*tc_count)].name = file_name;	/* Get path to the script from base dir */
			  ...
			  size_t *result_sz)
{
	...
	tests[0].name = strdup_check(name);		/* Contains the name/path from base dir of the script */
	tests[0].desc = desc;
	...
	for (i = 0; i < n_dirs && (ent = entlist[i]); i++) {
		...
		if (is_test_script(dir_fd, ent->d_name)) { /* It's a test */
			...
			if (desc) /* It has a desc line - valid script */
				append_script_to_suite(ent->d_name, desc, parent, tc_count, subpath);
			continue;
		}
		...
		if (asprintf(&new_path, "%s%s/", subpath, ent->d_name) < 0){
			continue;
		}
		append_scripts_in_subdir(fd, parent, tc_count, new_path);
		free(new_path); 			// cleanup, redo ugly
	}
				  ...
				  size_t *result_sz)
{
	...
	for (i = 0; i < n_dirs && (ent = entlist[i]); i++) {
		...
		append_scripts_in_subdir(fd, &test_suite, &cases_count, subpath);
		free(subpath);		// redo ugly?
		if (cases_count < 1){
			free_suite(test_suite, cases_count);
			continue;
		}

		if (is_test_script(fd, "setup.sh")) {
			char *desc = shell_test__description(fd, "setup.sh");
			test_suite->desc = desc;
			((struct shell_info*)(test_suite->priv))->has_setup = true;		// redo?
		}
		else {
